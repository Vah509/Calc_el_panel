...
Актуализирован под **v106** (2026-09-22) — TASK_ui_quick_fixes:
разделение кнопки "Сохранить" (движок форм теперь остаётся открытым
после сохранения, отдельная кнопка "Сохранить и закрыть" закрывает),
индикатор "требует сохранения"/"сохранено" в шапке формы, и починка
CSS hover-зоны выпадающего меню (см. подробности ниже и в HANDOFF.md).
Обновляется тем же шагом, где меняется код (то, что это правило
выпадало из процесса раньше и привело к сильному устареванию v48→v94
— сама причина ревизии v95).
...
### Универсальная форма движка: сохранение и индикатор состояния (v106)

Общий для ВСЕХ таблиц `modal-footer`/`modal-header` в `page.py`
(один Alpine-компонент `enginePage()`, разметка не дублируется по
таблицам):

- **`save()`** — отправляет `editing` на сервер (POST при создании,
  PUT при редактировании) и **оставляет форму открытой**. Мерджит
  ответ сервера в `editing` (важно при создании — подхватывает
  присвоенный сервером `id`, иначе повторное `save()` снова ушло бы
  POST'ом и создало бы дубликат) и обновляет `formSnapshot`
  (см. ниже).
- **`saveAndClose()`** — вызывает `save()`, и только при успехе
  закрывает форму тем же способом, что раньше делал старый `save()`
  целиком (`closeToList()` в form-режиме / `modalOpen = false` +
  `load()` в modal-режиме).
- **`close()`** — не изменился: просто закрывает форму, без
  сохранения.
- Кнопки в футере (обычные документы и hierarchy-таблицы
  kit_group/kit_section — оба `modal-footer-right` блока): «Закрыть»
  → `close()`, «Сохранить и закрыть» → `saveAndClose()`, «Сохранить»
  (primary) → `save()`.

Индикатор «Есть несохранённые изменения» / «Сохранено» в
`modal-header` (только для обычных форм-документов, не
items_modal-справочников):
- `formSnapshot` — `JSON.stringify(editing)` на момент открытия формы
  или последнего успешного `save()`; `isDirty` пересчитывается через
  `$watch('editing', ...)` (глубокий по умолчанию — реагирует на
  любое поле) в `init()`.
- `takeFormSnapshot()` вызывается в конце `openCreate()` (после
  подстановки автономера) и в конце `openEdit()` (после
  `radioActions`/`CONFIG.openEditAction` — иначе автоподгрузка данных
  при открытии формы (например `refresh_cost_totals` калькуляции)
  сразу показала бы «не сохранено»).
