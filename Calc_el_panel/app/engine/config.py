...
class TableConfig:
    ...
    soft_delete: bool = False      # True — модель таблицы имеет поле is_deleted.
                                    # Кнопка в модалке становится "Удалить"/"Отменить"
                                    # (переключает флаг, не удаляет строку), в первой
                                    # колонке списка появляется точка-индикатор статуса.
                                    # DELETE-эндпоинт при этом ставит/снимает is_deleted
                                    # вместо session.delete(). Физическое удаление —
                                    # отдельно, через обработчик (app/processors).
