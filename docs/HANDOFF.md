...
**v86 — счёт (invoice) как четвёртый документ цепочки. Задеплоено на
Railway, найден и исправлен баг с `is_default` (Postgres, не ловился
на SQLite) — см. ниже.**
...
**⚠️ Прод-баг 2026-08-30, исправлен, нужен новый деплой:** после
загрузки v86 на Railway создание фирмы (`POST /api/firm`, кнопка
«Сохранить» на `/firm-v2`) падало с 500 (`TypeError: Not a boolean
value: ''`). Причина — поле `is_default` было объявлено обычным
`FieldConfig(widget="text", readonly=True)`, из-за чего движок всё
равно включал его в форму создания; пустая форма присылала
`is_default: ""`, SQLModel/Pydantic на уровне Python это не отловил
(просто присвоил строку атрибуту), а Postgres на INSERT потребовал
настоящий boolean и упал. На SQLite (локальные тесты этой же сессии)
баг НЕ воспроизводился — SQLite не строгий с типами колонок, поэтому
тесты прошли, а прод сломался. Тот же класс проблемы, что уже
фиксировали раньше (SQLite vs Postgres trap), просто новая
разновидность — не отсутствие PRAGMA foreign_keys, а отсутствие
строгой типизации bool. **Исправление**: `is_default` полностью убран
из `fields=[]` у `firm_table` (не входит в `field_names()` вообще →
ни create, ни update его не трогают) — переключается ИСКЛЮЧИТЕЛЬНО
кнопкой «Сделать по умолчанию» (`_set_default_firm_handler`, прямое
присвоение `True`/`False` в Python, без похода через HTTP-форму).
Готово к упаковке и повторной заливке в `ai-updates/`.
...
- **`app/engine/tables.py`**:
  ...
  - `_set_default_firm_handler` — кнопка «Сделать по умолчанию» на
    ...
    коэрсить bool из `<select>`/`<radio>`/пустой формы, та же логика,
    что у is_deleted; отдельная кнопка снимает флаг со всех остальных
    фирм). **is_default полностью исключён из `fields=[]` (см. прод-
    баг выше) — не входит в `field_names()`, не сериализуется, не
    приходит и не уходит через обычный create/update.**
...
- **Тест (ad hoc, TestClient, sqlite)**: полный цикл фирма → клиент →
  ...
  `/firm-v2` рендерятся без ошибок (200, непустой HTML). **После
  прод-бага (см. выше) добавлена явная проверка**: `POST /api/firm` с
  формой ровно как в браузере (без `is_default` в payload) — 200 OK,
  ответ не содержит `is_default` вообще.
...
- **`is_default` у Firm нигде не виден в UI** — только через кнопку
  ...
  дефолтная — сознательный компромисс после прод-бага (см. выше):
  безопаснее полностью убрать поле с формы, чем городить показ read-
  only bool без нормальной движковой поддержки. Можно доделать позже
  отдельным движковым примитивом для boolean-полей (сейчас в движке в
  принципе нет универсального способа безопасно показать/редактировать
  `bool`-поле модели через обычный FieldConfig — то же ограничение
  обошли и здесь, и раньше у is_deleted через спецкейс).
- **Deploy pipeline пропустил Postgres-специфичный баг** — SQLite CI
  не ловит строгую типизацию колонок (тот же класс проблем, что и
  отсутствие PRAGMA foreign_keys, см. раздел «Ключевые находки» в
  архиве этого файла/git-истории). Стоит держать в голове при
  добавлении ЛЮБОГО нового bool-поля в форму — either полностью
  исключать его из `fields=[]` (как здесь), либо явно приводить типы
  в create_item/update_item на уровне движка (общее решение пока не
  предложено, отдельная тема для обсуждения).
- Не проверено на реальном устройстве после фикса — предыдущий прогон
  (см. лог 2026-08-30 10:07) был именно тем, что поймал баг; нужен
  повторный деплой v86 (с фиксом) и повторная проверка создания фирмы
  + полного цикла счёта на живом Railway/Postgres.
